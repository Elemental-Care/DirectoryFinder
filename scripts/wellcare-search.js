...
async function searchWellcareProvider(npi, options = {}) {
  const {
    planKey = 'medicare',
  ...
  const result = {
    success: false,
    npi,
    plan: `Wellcare ${planKey.charAt(0).toUpperCase() + planKey.slice(1)}`,
  ...
  try {
    ...
    await page.goto(`https://my.wellcare.com/x/findaprovider/${lineOfBusiness}/en/default/location`,
    ...
    if (!providers || providers.length === 0) {
      ...
      const basePath = path.join(artifactDir, `wellcare-debug-${npiStr}-${timestamp}`);
    ...
    const artifactDir = ensureArtifactsDir();
    const timestamp = new Date().toISOString().replace(/[:.]/g, '-');
    const basePath = path.join(artifactDir, `wellcare-${planKey}-${npiStr}-${timestamp}`);
...
async function main() {
  ...
  if (!npiArg) {
    console.error('Usage: node scripts/wellcare-search.js <NPI> [--plan=medicare|medicaid] [--location="City, ST or ZIP"]');
    process.exit(1);
  }

  let planKey = 'medicare';
  ...
  const result = await searchWellcareProvider(npiArg.trim(), options);
...
module.exports = { searchWellcareProvider };
